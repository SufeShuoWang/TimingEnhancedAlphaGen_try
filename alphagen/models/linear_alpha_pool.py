...
class LinearAlphaPool(AlphaPoolBase, metaclass=ABCMeta):
    ...
    def force_load_exprs(self, exprs: List[Expression], weights: Optional[List[float]] = None) -> None:  # 强制加载一批表达式，主要用于从之前的结果恢复
        self._failure_cache = set()
        old_ic = self.evaluate_ensemble()  # 计算当前池的IC作为对比基准
        old_pool: List[Expression] = self.exprs[:self.size] # type: ignore
        added = []
        for expr in exprs:
